...
rem
rem  NOTE: no parenthesized if-blocks below - the install paths contain
rem  "(x86)", which breaks cmd's (...) parser when the vars expand inside
rem  a block. goto labels are used instead.
rem ===========================================================================
...
if errorlevel 8 goto FAIL_ROBOCOPY
...
if errorlevel 1 goto FAIL_MAKEINI
...
:RC_OK
if exist "%P2SYS%\%PKG%.u" goto PKG_OK
